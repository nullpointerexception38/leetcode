      ...
      if (status[box] === 1) {
        ...
        for (const containedBox of containedBoxes[box]) {
          if (status[containedBox] === 1) {
            nextBoxes.push(containedBox)
          }
          else {
            nextClosedBoxes.push(containedBox)
          }
        }
      }
      else {
        nextClosedBoxes.push(box)
      }
    }
    for (const closedBox of closedBoxes) {
      if (status[closedBox] === 1) {
        nextBoxes.push(closedBox)
      }
      else {
        nextClosedBoxes.push(closedBox)
      }
    }
    boxes = nextBoxes
    closedBoxes = nextClosedBoxes
  }
  return sum
}
